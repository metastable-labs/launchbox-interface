  ...
  return (
    <div
      className={classNames(
        "self-stretch overflow-x-auto rounded-xl border border-primary-950 flex flex-col justify-between bg-white shadow-table-cta",
        {}
      )}
    >
          ...
          <tr>
            {headers.map((header, index) => (
              <th
                key={header.key}
                className={classNames(
                  "px-4 md:px-6 py-3 text-[12px] leading-[18px] font-medium whitespace-nowrap",
                  {
                    "w-[83px]": index === 0 && variant === "secondary",
                    "hidden md:table-cell":
                      (index === 2 || index === 3) && variant === "primary",
                    hidden:
                      (index === 2 || index === 3) && variant === "secondary",
                    "text-right":
                      index === headers.length - 1 && variant === "primary",
                    "text-left":
                      index !== headers.length - 1 || variant === "secondary",
                  }
                )}
              >
                {header[variant]}
        ...
        <tbody className="bg-white divide-y divide-gray-200 text-sm font-medium">
          {data.map((item, index) => (
            <tr key={index}>
              <td
                className={classNames(
                  "min-h-[71px] px-4 md:px-6 py-4 text-gray-900",
              ...
              >
                {variant === "primary" && (
                  <Address
                    wallet={item.wallet || "-"}
                    walletAvatarURL={item.walletAvatarURL}
                  />
                )}
              ...
              <td
                className={classNames(
                  "min-h-[71px] px-4 md:px-6 py-4 whitespace-nowrap text-gray-500",
              ...
              {variant === "primary" && (
                <td className="min-h-[71px] px-4 md:px-6 py-4 whitespace-nowrap text-gray-500">
                  <span className="text-primary-650">
                    {item.usdAmount ? "$" + formatNumber(item.usdAmount) : "-"}
                  </span>
                </td>
              )}
              ...
              {variant === "primary" && (
                <td className="min-h-[71px] px-4 md:px-6 py-4 whitespace-nowrap text-gray-500">
                  <span className="text-primary-650">
                    {item.tokenAmount &&
                      formatNumber(item.tokenAmount || 0) + " " + tokenSymbol}
                    {!item.tokenAmount && "-"}
                  </span>
                </td>
              )}
