    ...
    <div className="flex flex-col gap-[28px] min-w-[300px] md:min-w-80">
      ...
      <div className="flex flex-col md:min-w-80 flex-1 items-stretch gap-8">
        {networks?.map(({ icon, variant, chainId: id }, index) => (
          <div key={index} onClick={() => switchNetwork(id)} className="flex items-center justify-between hover:bg-primary-200 transition-colors duration-300 cursor-pointer py-2 px-5 -mx-5">
            <div className="flex items-center justify-center gap-2">
              {icon} <span className="capitalize text-[14px] leading-[21.7px] font-medium text-primary-150">{variant}</span>
            </div>
