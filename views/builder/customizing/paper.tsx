...
const CustomizingPaper = ({ icon, isActive, title, onClick }: ICustomizingPaper) => {
  ...
  return (
    <div
      onClick={onClick}
      className={classNames('min-w-full flex items-center justify-between self-stretch cursor-pointer', {
      ...
      <div className="flex items-center justify-center gap-1 min-h-6">
        <AnimatePresence mode="popLayout">
          <motion.div key={isActive ? 0 : 1} initial={{ opacity: 0 }} animate={{ opacity: 1, rotate: isActive ? 180 : 0 }} exit={{ opacity: 0 }} className="min-w-6 min-h-6">
            {icons[isActive ? 0 : 1]}
          </motion.div>
        </AnimatePresence>
