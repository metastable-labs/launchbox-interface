...
const Step = ({ step, title, current, passed, network, onClick }: StepProps) => {
  return (
    <div
...
const SecondaryHeader = ({ step, network, setStep }: { step: number; network: Network; setStep: (step: number) => void }) => {
  return (
    <AnimatePresence>
      {step < 2 && (
        <motion.div key={0} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center justify-center gap-4">
          ...
          <RightCarretDarkIcon />
          <Step current={step === 1} network={network} passed={step > 1} step={2} title="Deploy" />
        </motion.div>
      )}
    </AnimatePresence>
  );
