...
const Confirmation = ({ tokenData, network }: IConfirmation) => {
  ...
  return (
    <div className="min-w-full flex flex-col gap-6">
      <h1 className="text-primary-150 text-lg md:text-[24px] md:leading-[37.2px]">
        Confirmation
      </h1>
        ...
        {info.map(({ title, value, icon }, index) => (
          <div
            ...
            className={classNames(
              "w-full flex items-center justify-between text-base text-primary-700",
              { "pb-4 border-b border-primary-50": index !== info.length - 1 }
            )}
          >
            <span>{title}</span>
      ...
      <LBButton
        text={`Confirm and deploy $${tokenData?.tokenSymbol}`}
        fullWidth
        network={network}
        variant="plain"
        type="submit"
