...
const Overview = ({ tokenDetailData }: IOverview) => {
  ...
  const noLiquidityData = liquidityData.every(
    ...
    <div className="self-stretch w-full flex flex-col items-center justify-center gap-6">
      <div className="flex items-center justify-end self-stretch">
        <LBSecondarySelect
          ...
          onClick={(option) => setPeriod(option.value)}
      ...
      <div className="px-6 flex flex-col gap-3 self-stretch">
        <div className="flex items-center gap-3 w-full">
          ...
          />
          <span className="text-primary-150 font-semibold text-[20px] leading-[32px] tracking-[-0.2px]">
            {name}
          </span>
        </div>
        ...
        {infoData.map(({ title, value, hasPercentageBar }, index) => (
          <div
            ...
            className={classNames(
              "p-4 flex-1 rounded-base border border-primary-1200 bg-primary-2500 min-h-full",
              {
          ...
          >
            <div className="flex flex-col justify-center gap-1.5">
              ...
              <span className="text-primary-650 text-[24px] font-medium">
                {value}
              </span>
            </div>
            ...
            {hasPercentageBar && (
              <div className="w-full h-2 bg-primary-950 rounded">
                <motion.div
                  className="h-full bg-primary-1000 rounded"
                  initial={{ width: 0 }}
                  animate={{ width: value }}
                />
              </div>
      ...
      <p className="self-stretch flex-1 px-6 py-3 flex items-center rounded-xl bg-primary-300 text-[14px] leading-[24px] text-primary-2550">
        When the market cap reaches $100,000 all the liquidity from the bonding
        curve will be deposited into Aerodrome and burned. progression increases
